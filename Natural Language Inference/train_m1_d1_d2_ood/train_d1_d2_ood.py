...
def main():
    ...
    tokenizer = AutoTokenizer.from_pretrained(args.m1_model_path)
    data1 = load_mnli(file_path=train_file_path, tokenizer=tokenizer)
    data2_id, data2_ood = load_snli(file_path=train_file2_path, tokenizer=tokenizer, id_ood=id_ood)
    print(f'MNLI train length : {data1.len}')
    print(f'SNLI ID train length : {data2_id.len}')
    print(f'SNLI OOD train length : {data2_ood.len}')
    data1_sample_size = int((args.train_sample_percent/100)*data1.len)
    data2_id_sample_size = int((args.train_sample_percent/100)*data2_id.len)
    data1,_ = random_split(data1, [data1_sample_size, data1.len - data1_sample_size])
    data2_id,_ = random_split(data2_id, [data2_id_sample_size, data2_id.len - data2_id_sample_size])
    data = ConcatDataset([data1, data2_id, data2_ood])
    print(f'Train dataset length : {len(data)}')
    save_dataset(data, './train.pkl')

    model = MainModel.from_pretrained(args.m1_model_path, num_labels = 3, loss_fn = LossFunction())
