...
def main():
    ...
    #print(f"Time to generate QQP train prob file generation : {total_time}")

    start = time.time()
    paws_data = load_paws(file_path='./PAWS/train.tsv', tokenizer=tokenizer)
    paws_dataloader = DataLoader(paws_data, shuffle = False, batch_size=BATCH_SIZE)
    
    paws_train_accuracy, paws_pred_lst, paws_prob_lst = inference(model, paws_dataloader, tokenizer, device)
    #print(f'\tPAWS train accuracy : {paws_train_accuracy}')
    paws_output_file_path = os.path.join(input_path,'Predictions', 'PAWS', 'train_pred.txt')
    generate_prediction_file(paws_pred_lst, paws_output_file_path)
    generate_prob_file(paws_prob_lst, './Predictions/PAWS/prob_paws_train.txt')
    end = time.time()
    total_time = end - start
    print(f"Time to generate PAWS train prob file generation : {total_time}")
    ...
    generate_prediction_file(val_pred_lst, './Predictions/QQP/qqp_val_pred.txt')
    generate_prob_file(val_prob_lst, './Predictions/QQP/qqp_val_prob.txt')
    end = time.time()
    total_time = end - start
    print(f"Time to generate QQP val prob file generation : {total_time}")
